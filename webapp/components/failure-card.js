...
export function failureCard(failure) {
  const title = escapeHtml(failure.type || 'Falha registrada');
  const summary = escapeHtml(failure.summary || 'Sem descrição.');
  const authorName = escapeHtml(failure.authorName || 'Sem nome');
  const trainId = escapeHtml(failure.trainId || '-');

  return `
  <article class="failure-card" data-id="${escapeHtml(failure.id)}">
    <div class="body">
      ...
      <div class="footer">
        ...
        <button class="btn-link" data-open-details="${escapeHtml(failure.id)}">Ver detalhes</button>
      </div>
    </div>
  </article>`;
}
